...
_HEADERS = ["Branch", "Status", "Remote"]

_ANSI_RE = re.compile(r"\x1b\[[0-9;]*m")


def get_branch_tidy_status_pretty() -> int:
	column_widths = _get_column_widths()

	print(_render_row(_HEADERS, column_widths))
	...
	for branch, branch_status, merge_status in get_branch_tidy_status():
		print(
			_render_row(
				[
					branch,
					_BRANCH_STATUS_STRINGS.get(branch_status, branch_status.name),
					_MERGE_STATUS_STRINGS.get(merge_status, merge_status.name),
				],
				column_widths,
			)
		)
...
if __name__ == "__main__":
	...
	raise SystemExit(get_branch_tidy_status_pretty())
